...
class OutboxPublisherTest {
    ...
    @Test void sendsWholeBatchBeforeWaitingForPublisherConfirms() {
        var db=mock(JdbcTemplate.class); var rabbit=mock(RabbitTemplate.class);
        when(db.queryForList(anyString(),eq(Long.class))).thenReturn(List.of(41L,42L));
        when(db.update(anyString(),any(Timestamp.class),anyLong())).thenReturn(1);
        ...
        verify(db).update("UPDATE tb_outbox_event SET last_error=NULL WHERE id=?",41L);
        verify(db).update("UPDATE tb_outbox_event SET last_error=NULL WHERE id=?",42L);
    }
}
